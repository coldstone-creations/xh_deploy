...
///
/// Instantiated by InferenceNode with a callback that triggers a state-machine
/// transition to JointDamping.  Methods throw on violation to abort the current
/// inference tick; the callback ensures the state machine enters the safe state
/// on the next cycle.
///
...
public:
    ...
    /// @param joint_limits     Flattened lower/upper pairs per joint.
    /// @param on_violation     Called when a violation is detected (typically
    ///                         logs + ForceTransition to JointDamping).
